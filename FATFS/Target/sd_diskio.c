...
/* FIX: Persist changes across CubeMX generation */
#undef SD_TIMEOUT
#define SD_TIMEOUT 5000

/* Redefine BSP_SD_GetCardState to automatically inject osDelay(1) during busy-wait polling.
   This prevents AboveNormal TaskSD from starving Normal TaskGUI. */
static inline uint8_t BSP_SD_GetCardState_Persistent(void) {
  if (osKernelGetState() == osKernelRunning) osDelay(1);
  return BSP_SD_GetCardState();
}
#undef BSP_SD_GetCardState
#define BSP_SD_GetCardState() BSP_SD_GetCardState_Persistent()
